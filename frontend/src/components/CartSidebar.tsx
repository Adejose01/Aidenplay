...
export default function CartSidebar() {
  const { items, isCartOpen, setIsCartOpen, removeFromCart, clearCart, totalArs, totalRd, totalUsd } = useCart();
  const { country, whatsappNumber, settings, loading } = useSettings();
  ...
  const handleCheckout = () => {
    ...
    const regionName = country === 'AR' ? 'Argentina 🇦🇷' : country === 'DO' ? 'Rep. Dominicana 🇩🇴' : 'Internacional 🌍';
    const total = country === 'AR' ? `AR$ ${formatPrice(totalArs)}` : country === 'DO' ? `RD$ ${formatPrice(totalRd)}` : `US$ ${formatPrice(totalUsd)}`;
  ...
  const getCartTotalDisplay = () => {
    if (loading) return <div className="animate-pulse bg-white/10 h-8 rounded-xl w-32"></div>;
    
    if (country === 'AR') {
      return <span className="font-black text-lg text-neon-blue tracking-tighter">AR$ {formatPrice(totalArs)}</span>;
    } else if (country === 'DO') {
      return <span className="font-black text-lg text-neon-purple tracking-tighter">RD$ {formatPrice(totalRd)}</span>;
    } else {
      return <span className="font-black text-lg text-white tracking-tighter">US$ {formatPrice(totalUsd)}</span>;
    }
  };

  const getCountryName = () => {
    if (country === 'AR') return 'Argentina 🇦🇷';
    if (country === 'DO') return 'Rep. Dominicana 🇩🇴';
        ...
        {items.length > 0 && (
          <div className="p-6 border-t border-white/5 bg-black/40 space-y-6">
            ...
            >
              ...
              {loading ? 'Verificando Región...' : 'Proceder al Pago'}
            </button>
          </div>
        )}
      </div>
